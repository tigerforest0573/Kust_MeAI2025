...
def build_dataset(cfg: dict, split: str) -> ECGDataset:
    ...
    # 修改部分：使用单个 CSV 和 fold 进行数据切分
    df_name = cfg.get("all_data_csv", "train.csv")
    current_fold = cfg.get("current_fold", 0)
    
    full_df = pd.read_csv(os.path.join(index_dir, df_name))
    
    if split == "train":
        # 训练集使用除了当前 fold 以外的数据
        df = full_df[full_df['fold'] != current_fold].reset_index(drop=True)
    else:
        # 验证集或测试集使用当前 fold 的数据
        df = full_df[full_df['fold'] == current_fold].reset_index(drop=True)
