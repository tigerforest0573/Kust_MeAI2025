...
def main(config):
    ...
    if config['mode'] != "scratch":
        checkpoint = torch.load(config['encoder_path'], map_location='cpu', weights_only=False)
    ...
    if config['mode'] == "lora":
        ...
        peft_config = LoraConfig(
            ...
            lora_alpha=16,      
            lora_dropout=0.1,
            target_modules=["to_qkv", "to_gate", "to_out.0"], 
            modules_to_save=["head", "attn_pool"], 
        )
    ...
    best_loss = float('inf')
    best_challenge_score = -float('inf')
    ...
    for epoch in range(config['start_epoch'], config['train']['epochs']):
        ...
        #最高challenge_score保存
        curr_challenge_score = metrics.get('challenge_score', -float('inf'))
  
        if output_dir and curr_challenge_score > best_challenge_score:
            best_challenge_score = curr_challenge_score
            print(f"New Best Challenge Score: {best_challenge_score:.4f}")
            misc.save_model(
                config,
                os.path.join(output_dir, 'best-challenge_score.pth'),
                epoch,
                model_without_ddp,
                optimizer,
                loss_scaler,
                metrics={
                    'loss': curr_loss,
                    'challenge_score': curr_challenge_score,
                    **metrics
                }
            )
